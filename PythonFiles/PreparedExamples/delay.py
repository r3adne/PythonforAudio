...
def sample_delay(samples, n):
    ...
    for sample_number in range(len(samples) + n):  # sample_number = 0
        if sample_number <= n: 
            out.append(0)
        else:
...
def circbuff_delay(samples, time_seconds, SR, mix, feedback):
    ...
    read = write - n # n samples apart [write          read          ]
    
    for sample in samples: # 0
        # circbuff[read] delayed
        # circbuff[write] the sample we're about to set 
        circbuff[write] = sample + (feedback * circbuff[read])
        out.append((circbuff[read] * mix) + (sample * dry_mix))

        write += 1 # increment write  
        read += 1 # increment read
        
        # a % b  takes the remainder of a / b.
        write %= bufflength # if write is longer than the buffer's length, it will return it to the beginning of the buffer.
        # write = write % bufflength
        read %= bufflength # ""
...
x = circbuff_delay(y, 0.2, sr, 0.5, 0.9)
...
# x is output of delay
# write audio file to output.wav
librosa.output.write_wav('./output.wav', x, sr)